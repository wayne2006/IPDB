...
namespace IPDB
{
    ...
    public class IPSearch3Fast
    {
        ...
        private string datPath = AppContext.BaseDirectory + @"\Data\ip.db";
        private DateTime lastRead = DateTime.MinValue;

        private long[,] prefmap = new long[256, 2];
        private uint[] startArr;
        private uint[] endArr;
        ...
        private void LoadDat()
        {
            ...
            {
                long p = 2052 + (i * 12);
                uint startipnum = ReadLittleEndian32(data[p], data[1 + p], data[2 + p], data[3 + p]);
                uint endipnum = ReadLittleEndian32(data[4 + p], data[5 + p], data[6 + p], data[7 + p]);

                int offset = data[8 + p] + ((data[9 + p]) << 8) + ((data[10 + p]) << 16);
                int length = data[11 + p];

                startArr[i] = startipnum;
                endArr[i] = endipnum;
        ...
        private long BinarySearch(long low, long high, long k)
        {
            ...
            //判断IP是否在IP段中；如果IP段不连续，则找出的IP段可能不正确。
            if (startArr[M] > k) return -1;
        ...
        public long IpToInt(string ipString, out long prefix)
        {
            //高性能
